...
static agbsize_t find_loose(std::string_view rom, std::string_view pattern,
...
static agbsize_t find_backwards(std::string_view rom,
                               std::array<std::string_view, _Size> patterns,
                               agbsize_t pos, agbsize_t length) {
  if (pos >= rom.size()) return agbnpos;

  constexpr agbsize_t align = 4;
  assert(length % align == 0);
  if (length < align || rom.size() < length) return agbnpos;
